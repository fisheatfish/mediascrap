...
cursor = links.find({"name":"http://www.lequipe.fr/"},{"_id":0,"name":1,"liste_links":1})
...
class LequipeSpider(CrawlSpider):
    """
    ...
    def parse_page(self,response):
        ...
        title = ''.join(hxs.select('//strong[@class="nb-caractere-51"]//text()').extract()).strip()
        date_article = ''.join(hxs.select('//time[@itemprop="datePublished"]').extract()).strip()
        body = ''.join(hxs.select('//div[@class="paragr paragraf1"]//text()').extract()).strip()
        ...
        if len(body)> 0 :
            ...
            item['date_article'] = date_article[41:63]
            if len(title)> 0 :
                item['title']= title
            return item
